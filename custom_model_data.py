...
def fashion_mnist_loader(spec, use_bounds=False):
    eps = spec.get("epsilon")
    assert eps is not None, "You must specify an epsilon in the spec."
    ...
    test_data = datasets.FashionMNIST(root=dataset_path, train=False, download=True, transform=transform)
    ...
    if use_bounds:
        # Option 1: element-wise lower and upper bounds for each example
        new_eps = torch.reshape(eps / std, (1, -1, 1, 1))
        data_max = torch.min(X + new_eps, absolute_max)
        data_min = torch.max(X - new_eps, absolute_min)
        ret_eps = None
    else:
        # Option 2: global epsilon and clipping bounds
        data_max = absolute_max
...
def squeezenet_fashion_mnist():
    ...
    # Remove the pooling layer because it causes an error
    model.classifier[0] = nn.Identity()

    # Load the saved weights
    model.load_state_dict(torch.load('models/fashion_mnist_squeezenet/squeezenet_fashion_mnist2.pth'))
    # model.load_state_dict(torch.load('../models/fashion_mnist_squeezenet/squeezenet_fashion_mnist2.pth'))
